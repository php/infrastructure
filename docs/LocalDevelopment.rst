...
Ensure all machines have root SSH access enabled before starting. You must be able
to ``ssh root@<ip>`` from your machine to each Debian machine.

Step 1: Install Ansible
-----------------------

While you may be able to install Ansible from your operating system's package manager
the Pythonic way would be to use a virtual environment.You can use [virtualenv](https://virtualenv.pypa.io/en/latest/) or
...
Create `inventory/local/group_vars/all.yml` from `inventory/php/group_vars/all.yml.skeleton`
